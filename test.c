...
void genererNourriture(InfoJeux infos, Apple apple, int z) {
    int test = 0;
    apple.position[z].x = (rand() % infos.nbX) * infos.tailleCase;
    apple.position[z].y = (rand() % infos.nbY) * infos.tailleCase;
}

void deplacersnake(Direction direction, Snake snake, InfoJeux infos) {
...
void afficher(Snake snake, Apple apple, InfoJeux infos, int timerGame){
    ...
    for (int i = 0; i < apple.nbPomme; i++){
        if (apple.position[i].x == -1 && apple.position[i].y == -1) {
            genererNourriture(infos, apple, i);
        }
    }
...
int checkDeplacement(Snake snake, InfoJeux infos) {
...
int playCycle(Direction direction, Snake snake, Apple apple, InfoJeux infos){
...
int main(void) {
    ...
    direction = GAUCHE;
    afficher(snake, apple, infos, 0);

    playCycle(direction, snake, apple, infos);
